 ...
 */
package io.airxos.syniverse.api;
...
public class ContactGroup extends BaseData {
    ...
    static public class Resource extends ResourceBase {
        ...
        public String create(final ContactGroup contact) throws ScgException {
            return ExecuteWithRetry(() -> Session.Execute(contactGroupApi_.create(contact)).id, 0);
        }

        public boolean update(final ContactGroup contact) throws ScgException {
            ExecuteWithRetry(() -> Session.Execute(contactGroupApi_.update(contact.getId(), contact)), 0);
            return true;
        }
        ...
        public ContactGroup get(final String id) throws ScgException {
            return ExecuteWithRetry(() -> Session.Execute(contactGroupApi_.get(id)), 0);
        }
        ...
        public ItemItetaror<ContactGroup> list(Map<String, String> options) throws ScgException {
            return genericList(
                options,
                null,
                opts -> ExecuteWithRetry(() -> Session.Execute(contactGroupApi_.list(opts)), 0));
        }
        ...
        public boolean delete(final String id) throws ScgException {
            ExecuteWithRetry(() -> Session.Execute(contactGroupApi_.delete(id)), 0);
            return true;
        }
        ...
        public boolean addContact(final String groupId,
            ...
            ExecuteWithRetry(() -> Session.Execute(contactGroupApi_.addContact(groupId, payload)), 0);
            return true;
        }

        public boolean deleteContact(final String gid, final String cid) throws ScgException {
            ExecuteWithRetry(() -> Session.Execute(contactGroupApi_.deleteContact(gid, cid)), 0);
            return true;
        }

        public ItemItetaror<Contact> listContacts(final String id,
                Map<String, String> options) {
            return genericList(options, null, opts -> ExecuteWithRetry(() -> Session.Execute(contactGroupApi_.listContacts(id, opts)), 0));
        }

        public ItemItetaror<Contact> listContacts(final String id,
                Map<String, String> options,
                ListParameters parameters) {
            return genericList(options, parameters, opts -> ExecuteWithRetry(() -> Session.Execute(contactGroupApi_.listContacts(id, opts)), 0));
        }
    }
}
